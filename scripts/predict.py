...
import logging
import json
from .run_pipeline import main as start_run_pipeline
# Add src to path
...
def main():
    ...
    try:
        ...
        if not model_dir.exists() or not metrics_path.exists():
            print(f"Error: Model directory not found: {model_dir}", file=sys.stderr)
            print("Starting up the pipeline first, this will only trigger if model file was removed or corrupted: python -m scripts.run_pipeline", file=sys.stderr)
            start_run_pipeline()
        ...
        return result['predicted_net_gbp']
